...
//	Sourcecode which use TreeListWnd must be published. Commercial user
//	published their code too, or make an agreement with me.
//
//
//	TreeListWnd wird unter GNU GENERAL PUBLIC LICENSE (GPL) vertreiben.
//	Sourcecode welcher TreeListWnd muss veröffentlicht werden. Kommerzielle 
//	Nutzer müssen iren Code ebenfalls veröffentlichen, oder eine Nutzungs-
//	vereinbarung mit mir treffen.
//
...
LPCSTR		pText;
	...
	sRect.left    = 0;
	sRect.right   = iWidth;
	iYpos		  =(iHeigh-pData->iFontHeigh)/2;
...
RECT	sRect;
	...
	if(iHeight<pData->iFontHeigh  +2)iHeight=pData->iFontHeigh  +2;
	...
	if(uFlags&TVHT_ONITEM)
		...
		if(pData->uToolTipItem!=uItem || pData->uToolTipSub!=0)
			...
			if(sRect.right-sRect.left<=pEntry->iTextPixels+4)
				...
				SendMessage   (pData->hToolTip,WM_SETFONT,(WPARAM)hFont,0);
				SetWindowText (pData->hToolTip,pText);
				SetWindowPos  (pData->hToolTip,HWND_TOP,sRect.left+1,sRect.top+1,iPixels+5,pData->iFontHeigh+3,SWP_SHOWWINDOW);
				RedrawWindow  (pData->hToolTip,NULL,NULL,RDW_INVALIDATE|RDW_UPDATENOW|RDW_ERASE|RDW_FRAME);
...
unsigned	uSub;
	...
	if(pData->hFontN!=pData->hFontL)
		...
		pData->iFontHeigh =  sMetrics.tmHeight;
...
int			iDel;
	...
	if(pTemp)												// Den alten Eintrag abwählen
		{
		if(pTemp->uShowPos)									// Den Eintrag neu zeichen
			{
			if(pData->uStyleEx&TVS_EX_FULLROWMARK)
					UpdateRow (pData,uOld);
			else	UpdateRect(pData,uOld,uSub);
	...
	if(uItem)												// Den neuen Eintrag wählen
		...
		if(pEntry->uShowPos)								// Den Eintrag neu zeichen
			{
			if(pData->uStyleEx&TVS_EX_FULLROWMARK)
					UpdateRow (pData,uItem);
...
unsigned	uSub;
	...
	if((unsigned)iYpos>=pData->uSizeX)
		{
		pInfo->hItem = NULL;
	...
	if(iXpos>=pData->iColumnXpos[1])						// Aus Extraeintrag
		...
		for(uSub=1;uSub<pData->uColumnCount;uSub++)
			...
			else if(!pExtra || iXpos<pExtra->iTextPixels+5)	// Auf Text
				{
				pInfo->flags  = TVHT_ONSUBLABEL;
				}
			else{											// Rechts von Text
...
int				iDel;
	...
	if(GetAsyncKeyState(VK_CONTROL)&0x8000)
		...
		switch(wParam)
			{
		case VK_END:	iLine  = pData->uTreeItemsCount;
						iLine -= pData->uPageEnties;
						break;

		case VK_HOME:	iLine=0;
						break;

		case VK_LEFT:	iSub-=16;
						break;

		case VK_RIGHT:	iSub+=16;
						break;
		...
		case VK_SPACE:	if(pEntry && (pData->uStyle&TVS_CHECKBOXES))
							{
							TV_ITEM	 sItem;
...
BaseItem  **pList;
	...
	if(!pEntry)return 0;
	if(!pEntry->uFirstChild)return 1;
	if( pEntry->uFirstChild==pEntry->uLastChild)return 1;


	if(iMode)												// Sortiere die Untereinträge
		{
		pNext = pList[pEntry->uFirstChild];

		do	{
			TreeListSortItemsCb(pData,pSortData,SORT_NOUPDATE);
...
int				iTemp;
	...
	if(!pData->hEdit)										// Editfenster neu erzeugen
		{
			pData->hEdit=CreateWindow("EDIT",NULL,WS_BORDER|WS_CHILD|ES_AUTOHSCROLL|ES_LEFT,0,0,0,0,pData->hWnd,NULL,NULL,NULL);
		if(!pData->hEdit)return NULL;

		pData->pEditProc = (WNDPROC)GetWindowLongPtr(pData->hEdit,GWL_WNDPROC);

		SetWindowLongPtr(pData->hEdit,GWL_USERDATA,(LPARAM)pData);
	...
	SetWindowPos (pData->hEdit,HWND_TOP,sRect.left+2,sRect.top+1,iPixels+10,pData->iFontHeigh+4,SWP_SHOWWINDOW);
...
HDC				hDc;
	...
	case WM_PAINT:											// Das Fenster zeichen
	...
	case WM_COMMAND:										// Kommando Nachrichnten
		...
		if(wParam==MAKELONG(3,EN_KILLFOCUS) || wParam==MAKELONG(3,EN_RETURN))
			{
			   pData = GetHandle(hWnd);
			if(pData->uEditItem)
				{
	...
	case WM_GETDLGCODE:										// Welsche Tasten werden im Dialog benutzt
...
//******************** Einträge zeichen ***************************************
	...
	for(;uPos<uMax;uPos++)									// Alle Einträge ausgeben
		...
		else if(uStyle&TVS_HASLINES)						// Nur Linien zeichen ohne Schaltflächen
			{
			sArea.right = sArea.left+iIdent;
			ExtTextOut(hDc,0,0,ETO_OPAQUE,&sArea,NULL,0,NULL);
...
NoRootLines:
		...
		if(uStyleEx&TVS_EX_ITEMLINES)						// Linien um den Eintrag zeichen
			{
			iAdd		= 1;
		...
		if(pData->hImages && iImage!=TV_NOIMAGE)			// Icon zeichen vom Haupteintrag
			{
		...
		iYpos		= sArea.top+(iHeight-pData->iFontHeigh)/2;
...
ExtraDraw:
		...
		for(uExtra=1;uExtra<=pData->uColumnCount;uExtra++)	// Extra Spalten zeichen
			{
			...
			if(uStyleEx&TVS_EX_ITEMLINES)					// Linie um den Eintrag zeichen
				{
				SetBkColor(hDc,uFrColor);
			...
			if(!pExtra)
				...
				if(pData->hImages && iImage>TV_NOIMAGE)	// Icon zeichen
					{
