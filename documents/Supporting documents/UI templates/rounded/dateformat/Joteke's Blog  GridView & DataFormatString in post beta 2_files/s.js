...
function gAT(e){oTop=e.offsetTop;while(e.offsetParent!=null){oParent=e.offsetParent;oTop+=oParent.offsetTop;e=oParent;}
return oTop;}
function FP(){}
FP.IsVersionSupported=function(rVer){var ver=0;if(navigator.plugins&&navigator.plugins.length>0){var plugin=navigator.plugins["Shockwave Flash"];if(plugin){if(plugin.description){var re=new RegExp("-?[0-9]+\.[0-9]+");ver=parseInt(plugin.description.match(re)[0]);}}}else{try{var obj=new ActiveXObject("ShockwaveFlash.ShockwaveFlash."+rVer);if(typeof(obj)!='undefined'){ver=rVer;}}catch(e){}}return ver>=rVer;};(function(){function CI(){try{var w=window;var d=document;if(!w.AvoidTypes){var av=gC('lq_avoid');if(av!=null&&av!=''){w.AvoidTypes=av;}else{var isFlashOk=FP.IsVersionSupported(6);if(isFlashOk)w.AvoidTypes=0;else w.AvoidTypes=1;sC('lq_avoid',w.AvoidTypes,1);}}
...
function CMA(){var w=window;if(w.lqm_format<16||w.lqm_format==18)
...
function BU()
{var w=window;if(!w.lqm_pageid)w.lqm_pageid=gR();w.lqm_url="http://a.lakequincy.com/i.ashx?";if(!CMA())return;QS("channel",w.lqm_channel);QS("country",w.lqm_country);QS("debug",w.lqm_debug);QS("format",w.lqm_format);QS("height",w.lqm_height);QS("placement",w.lqm_placement);QS("creative",w.lqm_creative);QS("pageid",w.lqm_pageid);QS("publisher",w.lqm_publisher);QS("servertime",w.lqm_servertime);QS("target",w.lqm_target);QS("userguid",w.lqm_userguid);QS("width",w.lqm_width);QS("ypos",w.lqm_ypos);QS("zone",w.lqm_zone);QS("ct",w.lqm_total_ads);QS("avoidTypes",w.AvoidTypes);document.write('<script type="text/javascript" src="'+w.lqm_url+'"></'+'script>');}window.lqm_ypos=-1;CI();BU();RP();})()