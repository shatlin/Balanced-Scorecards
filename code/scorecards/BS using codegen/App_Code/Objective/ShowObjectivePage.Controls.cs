...
// ShowObjectivePage.aspx page.  The Row or RecordControl classes are the 
...
namespace BS.UI.Controls.ShowObjectivePage
{
...
public class ObjectiveRecordControl : BaseObjectiveRecordControl
{
      
        // The BaseObjectiveRecordControl implements the LoadData, DataBind and other
...
// Base class for the RoleMeasureTableControlRow control on the ShowObjectivePage page.
...
public class BaseRoleMeasureTableControlRow : BS.UI.BaseApplicationRecordControl
{
        ...
        public virtual void SaveData()
        {
            ...
            // Objective in ObjectiveRecordControl is One To Many to RoleMeasureTableControl.
                    
            // Setup the parent id in the record.
            ObjectiveRecordControl recObjectiveRecordControl = (ObjectiveRecordControl)this.Page.FindControlRecursively("ObjectiveRecordControl");
            if (recObjectiveRecordControl != null && recObjectiveRecordControl.DataSource == null) {
                // Load the record if it is not loaded yet.
                recObjectiveRecordControl.LoadData();
            }
            if (recObjectiveRecordControl == null || recObjectiveRecordControl.DataSource == null) {
            ...
            this.DataSource.ObjectiveId = recObjectiveRecordControl.DataSource.ObjectiveId;
        ...
        protected override object SaveViewState()
        {
            ...
            string url = @"../Measure/ShowMeasurePage.aspx?Measure={RoleMeasureTableControlRow:FK:FK_RoleMeasure_Measure}";
            bool shouldRedirect = true;
            string TargetKey = null;
            string DFKA = null;
            string id = null;
...
// Base class for the RoleMeasureTableControl control on the ShowObjectivePage page.
...
public class BaseRoleMeasureTableControl : BS.UI.BaseApplicationTableControl
{
    ...
    // Setup default pagination settings.
        ...
        ObjectiveRecordControl parentRecordControl = (ObjectiveRecordControl)(this.Page.FindControlRecursively("ObjectiveRecordControl"));
            ObjectiveRecord parentRec = parentRecordControl.GetRecord();
            if (parentRec == null) {
                // Get the error message from the application resource file.
                throw new Exception(Page.GetResourceValue("Err:ParentNotInitialized", "BS"));
            }
           
            if (parentRec.ObjectiveIdSpecified) {
                wc.iAND(RoleMeasureTable.ObjectiveId, BaseFilter.ComparisonOperator.EqualsTo, parentRec.ObjectiveId.ToString());
            } else {
                wc.RunQuery = false;
                return wc;
            }
        ...
        public virtual WhereClause CreateWhereClause(String searchText, String fromSearchControl, String AutoTypeAheadSearch, String AutoTypeAheadWordSeparators)
        {
            ...
            if (recId != null) {
                if (KeyValue.IsXmlKey(recId)) {
                    ...
                    wc.iAND(RoleMeasureTable.ObjectiveId, BaseFilter.ComparisonOperator.EqualsTo, pkValue.GetColumnValue(ObjectiveTable.ObjectiveId).ToString());
                
                } else {
                
                    wc.iAND(RoleMeasureTable.ObjectiveId, BaseFilter.ComparisonOperator.EqualsTo, recId);
                ...
                foreach (System.Web.UI.WebControls.RepeaterItem repItem in rep.Items)
                {
                    ...
                    if (recControl.Visible && recControl.IsNewRecord) {
                        ...
                        if (recControl.OrgRoleId.Text != "") {
                            rec.Parse(recControl.OrgRoleId.Text, RoleMeasureTable.OrgRoleId);
                        }
                        if (recControl.MeasureId.Text != "") {
                            rec.Parse(recControl.MeasureId.Text, RoleMeasureTable.MeasureId);
                        }
        ...
        // Generate the event handling functions for button events.
            ...
            ((ObjectiveRecordControl)(this.Page.FindControlRecursively("ObjectiveRecordControl"))).ResetData = true;
...
#region "Helper Functions"
        ...
        public RoleMeasureTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (RoleMeasureTableControlRow[])(selectedList.ToArray(Type.GetType("BS.UI.Controls.ShowObjectivePage.RoleMeasureTableControlRow")));
        ...
        public RoleMeasureTableControlRow[] GetRecordControls()
        {
            ...
            return (RoleMeasureTableControlRow[])recList.ToArray(Type.GetType("BS.UI.Controls.ShowObjectivePage.RoleMeasureTableControlRow"));
        }
...
// Base class for the ObjectiveRecordControl control on the ShowObjectivePage page.
// Do not modify this class. Instead override any method in ObjectiveRecordControl.
public class BaseObjectiveRecordControl : BS.UI.BaseApplicationRecordControl
{
        public BaseObjectiveRecordControl()
        {
            this.Init += new EventHandler(Control_Init);
            this.Load += new EventHandler(Control_Load);
            this.PreRender += new EventHandler(Control_PreRender);
        ...
        // To customize, override this method in ObjectiveRecordControl.
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
            // Register the event handlers.
        
              this.ObjectiveDialogEditButton.Click += new ImageClickEventHandler(ObjectiveDialogEditButton_Click);
        }
        ...
        // Read data from database. To customize, override this method in ObjectiveRecordControl.
        public virtual void LoadData()  
        {
            ...
            ObjectiveRecord[] recList = ObjectiveTable.GetRecords(wc, null, 0, 2);
            if (recList.Length == 0) {
                throw new Exception(Page.GetResourceValue("Err:NoRecRetrieved", "BS"));
            }

            
                    this.DataSource = ObjectiveTable.GetRecord(recList[0].GetID().ToXmlString(), true);
                  
        }

        // Populate the UI controls using the DataSource. To customize, override this method in ObjectiveRecordControl.
        public override void DataBind()
        {
            base.DataBind();
                ...
                if(formattedValue != null){
                    ...
                    if (formattedValue.Length >= popupThreshold) {
                         ...
                         formattedValue = "<a OnClick=\'gPersist=true;\' OnMouseOut=\'detailRolloverPopupClose();\'" +
                              "OnMouseOver=\'SaveMousePosition(event); delayRolloverPopup(\"PageMethods.GetRecordFieldValue(\\\"BS.Business.ObjectiveTable, App_Code\\\",\\\"" +
                              this.DataSource.GetID().ToString() + "\\\", \\\"ObjectiveName\\\", \\\"Objective Name\\\"," +
                              " false, 200," +
            ...
            if (this.ObjectiveName.Text == null ||
                this.ObjectiveName.Text.Trim().Length == 0) {
                this.ObjectiveName.Text = "&nbsp;";
            }
        ...
        public virtual void SaveData()
        {
            ...
            // 2. Validate the data.  Override in ObjectiveRecordControl to add custom validation.
            this.Validate();

            // 3. Set the values in the record with data from UI controls.  Override in ObjectiveRecordControl to set additional fields.
            this.GetUIData();
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            string recId = this.Page.Request.QueryString["Objective"];
                
            if (recId == null || recId.Length == 0) {
                // Get the error message from the application resource file.
                throw new Exception(Page.GetResourceValue("Err:UrlParamMissing", "BS").Replace("{URL}", "Objective"));
            }
            ...
            if (KeyValue.IsXmlKey(recId)) {
                ...
                wc.iAND(ObjectiveTable.ObjectiveId, BaseFilter.ComparisonOperator.EqualsTo, pkValue.GetColumnValue(ObjectiveTable.ObjectiveId).ToString());
            } else {
                
                wc.iAND(ObjectiveTable.ObjectiveId, BaseFilter.ComparisonOperator.EqualsTo, recId);
            }
        ...
        protected override object SaveViewState()
        {
            ...
            return base.SaveViewState();
              ...
              // event handler for ImageButton
              public virtual void ObjectiveDialogEditButton_Click(object sender, ImageClickEventArgs args)
              {
        ...
        public String RecordUniqueId {
            get {
                return (string)this.ViewState["BaseObjectiveRecordControl_Rec"];
            }
            set {
                this.ViewState["BaseObjectiveRecordControl_Rec"] = value;
            }
        }
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.Literal ObjectiveDialogTitle {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ObjectiveDialogTitle");
            }
        }
        
        public System.Web.UI.WebControls.ImageButton ObjectiveDialogEditButton {
            get {
                return (System.Web.UI.WebControls.ImageButton)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ObjectiveDialogEditButton");
            }
        }
           
        public System.Web.UI.WebControls.Literal ObjectiveName {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ObjectiveName");
            }
        }
